...
d3.json("texas-map.json", function(error, texas) {
  g.append("g")
    .attr("id", "countries")
    .selectAll("path")
    .data(topojson.feature(texas, texas.objects.counties_2010).features)
    .enter()
    .append("path")
...
function generateracepie(d){
  ...
  .attr('height', height)
  .append('g')
  .attr('transform', 'translate(' + ($("#pie-graph").width() / 2) +  ',' + ($("#pie-graph").height() / 2.8 ) + ')');

  var arc = d3.svg.arc()
  .innerRadius(innerRadius)
  .outerRadius(function (d) { 
            return innerRadius + 30 + (d.data.size/5); 
          });
  ...
  .text(function(d) { return d + " - " + parseFloat(save[d]/total_num*100).toFixed(2) + "%"; });
}
...
function find_common_words(d) {
  ...
  var brokens = {"LL":"Y'ALL", "WASN":"WASN'T", "DON":"DON'T", "DIDN":"DIDN'T"}
...
function generatecloud(dataset){
  console.log($("#word-cloud").width());
  d3.layout.cloud().size([$("#word-cloud").width(), 600])
    ...
    var cloud = d3.select("#word-cloud").append("svg")
      ...
      .attr("height", 600)
      .append("g")
      .attr("transform", "translate(" + $("#word-cloud").width()/2 + ",400)")
      .selectAll("text")
      .data(words)
